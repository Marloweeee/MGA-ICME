...
def collate_fn(raw_batch,pr=False):
    ...
    # raw_query= raw_batch[5]
    img_features = torch.tensor(np.array(raw_batch[5]))
    txt_features=torch.tensor(np.array(raw_batch[6]))

    batch = [img_data, text_data, bbox, img_features,txt_features]
    return tuple(batch)
