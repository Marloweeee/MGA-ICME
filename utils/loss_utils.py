...
def get_pseudo_labels(visual_teacher_features, text_teacher_features):
...
def MGA_Loss(text_cls, visu_src, patch_mask,text_pseudo_features, visual_pseudo_features, args):

    logits_per_img,logits_per_txt,logits_img_img,logits_txt_txt=get_similarity(visu_src, text_cls, patch_mask)
    pseudo_label_image, pseudo_label_text=get_pseudo_labels(text_pseudo_features, visual_pseudo_features)
    
    # PSA loss
    psa_loss=PairedSampleAlignmentLoss(logits_per_img, logits_per_txt, args.psa_tau)

    # CSA KL loss
    csa_loss=KLAlignmentLoss(logits_per_img, pseudo_label_image, args.csa_tau, args.ct_tau, use_loss='kl')
    csa_loss+=KLAlignmentLoss(logits_per_txt, pseudo_label_text, args.csa_tau, args.ct_tau, use_loss='kl')

    # USA KL loss
    usa_loss=KLAlignmentLoss(logits_img_img, pseudo_label_image, args.usa_tau, args.ut_tau, use_loss='kl')
    usa_loss+=KLAlignmentLoss(logits_txt_txt, pseudo_label_text, args.usa_tau, args.ut_tau, use_loss='kl')

    # LA loss
    la_loss=LocalizationAlignmentLoss(visu_src, text_cls, patch_mask, args.la_tau)

    mga_loss = psa_loss+args.alpha * csa_loss + args.beta * usa_loss + args.gama * la_loss
